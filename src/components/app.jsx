...
import { extractRGB, combineRGB } from "../color";
...
function FooterButton(props) {
  return (
    <Button {...props} className="shadow-1 shadow-hover-2 shadow-up-hover" />
  );
}
...
const App = class extends React.Component {
  ...
  colorChange = (color) => {
    ...
    if (this.state.webhookMode) {
      try {
        const parsed = JSON.parse(input);
        parsed.embeds.forEach((entry) => {
          entry.color = val;
        });
        this.setState({ input: JSON.stringify(parsed, undefined, 2) }, () => {
          this.validateInput(this.state.input, this.state.webhookMode);
        });
        return;
      } catch (ignored) {}
  ...
  render() {
    const webhookModeLabel = `${
      this.state.webhookMode ? "Dis" : "En"
    }able webhook mode`;
    const themeLabel = `${this.state.darkTheme ? "Light" : "Dark"} theme`;
    const compactModeLabel = `${
      this.state.compactMode ? "Cozy" : "Compact"
    } mode`;
    const colorPickerLabel = `${
      !this.state.colorPickerShowing ? "Open" : "Close"
    } color picker`;
      ...
      <main className="vh-100-l bg-blurple open-sans">
        <div className="h-100 flex flex-column">
          ...
          <footer className="w-100 pa3 tc white">
            ...
            <FooterButton
              label={webhookModeLabel}
              onClick={this.toggleWebhookMode}
            />
